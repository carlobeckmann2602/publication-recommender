...
rec_app.engines = initial_engines

rec_app.model_path = "./ai_backend/data/generated_data/"


@rec_app.get("/models/possible")
def get_model_names() -> Dict[str, List[str]]:
    ...
    **return**: a list of possible model names
    """
    models = next(os.walk(rec_app.model_path))[1]
    return {"disk_models": models}


@rec_app.get("/{model_name}/load/")
def load_recommender(model_name: str,
    ...
    **return**: The currently loaded model after calculation
    ...
    recommender.load(path=rec_app.model_path, model_name=model_name)
    rec_app.engines[model_name] = recommender
    return {"loaded": model_name}


@rec_app.get("/{model_name}/match_id/{publication_id}/")
async def get_recommendation(model_name: str, publication_id: str, amount: int = 5):
    ...
    **return**: The found matches plus additional information like the used tokens, the distances and anny indexes
    """
    if model_name not in rec_app.engines.keys():
        raise HTTPException(status_code=404, detail="No model with this name loaded")
    recommender = rec_app.engines[model_name]
...
@rec_app.get("/{model_name}/match_token/{token}/")
...
@rec_app.get("/{model_name}/random_id/")
async def get_random(model_name: str):
    ...
    **return**: A random publication
    ...
    if model_name not in rec_app.engines.keys():
        raise HTTPException(status_code=404, detail="No model with this name loaded")
    publication = rec_app.engines[model_name].mapping.sample()[[Recommender.PUB_ID_KEY, Recommender.ANNOY_INDEX_KEY]]
    return publication.to_dict()


@rec_app.get("/models/loaded/")
async def get_current_recommender():
    ...
    **return**: The current model (recommendation engine)
    """
    return {"current_model": str(list(rec_app.engines.keys()))}
