...
class ArxivApiCrawler:
    ...
    def run(self):
        ...
        id_patterns = ArxivApiCrawler.id_patterns
        id_max_is_old = False
        id_min_is_old = False
        ...
        if id_max is not None and id_new is not None:
            if id_max != id_new:
                ...
                if yy_max == yy_new and mm_max == mm_new:
                    ...
                    update_list += self.crawl_metadata(start_at=0, max_results=ArxivApiCrawler.interval, descending=True)

        # get the next x publications after publication count in db
        update_list += self.crawl_metadata(start_at=len(id_list), max_results=ArxivApiCrawler.interval, descending=True)
    ...
    def update_db_entries(self, update_list):
        return self.write_csv(ArxivApiCrawler.dataset_path+"dataset.csv", update_list)
