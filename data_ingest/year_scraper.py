...
if __name__ == '__main__':
    ...
    if len(state_data) == 0:
        for month in range(1, 13):
            state_data.append({"year":year, "month": month, "current_max_num": 0, "final_max_num": 0})  
    ...
    for month in range(1,13):
        ...
        block_size = 100
        while True:
            num_idx, id_block = base.create_id_block(year, month, num_idx, block_size) # returns empty id_block wenn bereits in db
            if len(id_block) > 0:
                if len(id_block) == 1 and int(id_block[0][5:]) == 0:
                    continue
                print("\n- collecting publication metadata from arxiv ...")
                metadata_block = base.get_metadata_list(id_block)
                if len(metadata_block) > 0:
                    max_num = base.get_max_num(metadata_block) # update max_num mit höchster id aus metadaten block
                ...
                else: # leere arxiv api response bedeutet 
                    if max_num == 0: # es sind bisher keine für diesen monat erschienen
                        state_data = base.save_state_data(year, month, max_num, state_data, False)
                    else: # max_num wurde für diesen monat erreicht
